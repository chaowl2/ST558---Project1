...
for (i in seq_along(num_var)){
    mean_i <- sum(my_tibble[i] * my_tibble$PWGTP)/sum(my_tibble$PWGTP)
    sd_i <- sqrt(sum(my_tibble[i]^2*my_tibble$PWGTP)/sum(my_tibble$PWGTP)-mean_i^2)
    results <- c(results, mean_i, sd_i)
    }
for (i in seq_along(cat_var)){
    
    count_i <- 
    results <- c(results, count_i)
    }
