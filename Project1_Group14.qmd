...
plot.census <- function(x, num_var, cat_var, ...) {
  # coerce numeric and keep complete cases for value & weight
  dat <- x
  dat[[num_var]] <- suppressWarnings(as.numeric(dat[[num_var]]))
  dat <- dat[!is.na(dat[[num_var]]) & !is.na(dat$PWGTP), , drop = FALSE]
  if (!is.factor(dat[[cat_var]])) dat[[cat_var]] <- factor(dat[[cat_var]])
...
```{r}
plot.census(df1, num_var = "AGEP", cat_var = "SEX")
```
...
plot.census(df1, num_var = "AGEP", cat_var = "SCH")
```
