...
The following function is to query the Public Use Microdata Sample (PUMS) Census API. But first, we need to define the values for codes in the JWAP and JWDP variables.
...
-   0 : N/A, which means individuals were male, <15 years old, or >15 years old.
...
```
 ...
 Visually, the data between years 2010 and 2022 do not appear to be significantly different. 
 However,there is a slight increase in the average age from 2010 to 2022 in those who gave birth in the past 12 months (FER code 1). 
 The minimum age and maximum age of those who gave birth within the past 12 months for both years appear to be approximately 15 and 50 years old, respectively, with some possible outliers around age 50 (right above the max). 

Overall, the functions to query the census API, summarize the numerical and categorical variables, and plot them, provide ways to transform raw data from the PUMS Census into understanding, insight, and knowledge. 