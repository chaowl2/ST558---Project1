...
# Example
df1 <- query_multiyear_census(census_years = c("2019", "2022"), num_var = c("AGEP", "GASP","GRPIP"))
df1
```
...
for (i in seq_along(num_var)){
    sample_mean_i <- sum(my_tibble[i] * my_tibble$PWGTP)/sum(my_tibble$PWGTP)
  }
  return(list(mean_num_var = sample_mean_i))
}
```

Test summarizing function
```{r}
summary.census(df1, num_var = c("AGEP", )
```
